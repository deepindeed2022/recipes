...
int main(int argc, char const *argv[])
{
	...
	while (1)
	{
		...
		if((pIpheader->ip_p) == IPPROTO_TCP && lentcp != 0)
		{
			...
			printf("\n\n---------package[%i]=%dbytes-------", pCount, lentcp);
			printf("\n*IP protocol identifier:%i\n",ntohs(pIpheader->ip_id));
			printf("<src IP:%s dest IP:%s>",szSourceIP, szDestIP);
			printf("\ntotal length:%i",ntohs(pIpheader->ip_len));
